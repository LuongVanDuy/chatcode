...
1. `prepare_task(project, request)`
   ...
   - returns ranked relevant file contents, framework/WordPress context, symbols/relations and a compact Git baseline;
   - returns durable per-project rules that the user explicitly confirmed in earlier completed tasks;
   - returns verification hints derived from the project.
2. ChatGPT creates a standard unified diff from that packet.
3. `complete_task(task_id, patch, verify_commands, remember_project_rules)`
   ...
   - infers `php -l` and `node --check` for changed PHP/JavaScript files when explicit verification commands are omitted; on Windows, the Safe task runner can discover PHP from `PHP_HOME`, `LARAGON_ROOT`, a project's Laragon `www` ancestor, or common Laragon/XAMPP folders;
   - optionally stores explicitly confirmed project conventions without adding another MCP tool;
...
- Project memory rejects credential/secret fields and URLs, is capped at 24 rules per project, and is written only after successful finalization.
...
2. `prepare_task` returns source content, Brain/framework context, compact Git baseline, saved project rules, verification hints and a task id.
...
10. Omitted verification commands infer syntax checks for changed PHP/JavaScript files, and confirmed project rules are returned by later tasks for the same project only.
