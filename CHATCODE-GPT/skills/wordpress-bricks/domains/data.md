...
5. One-time work must have an end state: explicit setup/database action or guarded one-shot path, then no-op. Never leave migration/seed logic on normal frontend `init`/`wp` requests.
