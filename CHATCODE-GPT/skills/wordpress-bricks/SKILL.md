...
`LOCK TARGET -> prepare_task -> skill receipt -> prove task evidence -> owner-scoped change -> complete_task -> PASS -> STOP`
...
- Existing **Bricks element IDs** used by selectors/query targets/migrations must come from the current task persisted tree. Chat history, clones, frontend DOM/export and other projects do not count.
- Numeric **WordPress media IDs** introduced by a patch must be verified as live attachments on the current project. URL/file name or old task output is not evidence.
- Prefer semantic/global classes. `#brxe-*` is task-evidence checked; `[data-field-id]` coupling requires explicit user scope.
- New top-level PHP symbols receive duplicate-owner preflight; duplicate hook candidates are reported.
