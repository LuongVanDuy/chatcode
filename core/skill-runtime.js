...
function classifyWooCommerceTask(request, inspect = null) {
  ...
  const excluded = /\bnon[-\s]?(?:woocommerce|woo)\b|(?:không|khong|without|\bno\b|\bnot\b)[^\n]{0,50}(?:woocommerce|\bwoo\b)|(?:woocommerce|\bwoo\b)[^\n]{0,50}(?:không|khong|without|\bnot\b)|(?:cpt|custom\s+post\s+type|post\s+type)[^\n]{0,100}(?:không|khong|without|\bnot\b)[^\n]{0,40}(?:woocommerce|\bwoo\b)/.test(text);
  ...
  // Production routing has an inspection. In that path, generic "product"
  // language only means Woo when the project actually detected WooCommerce.
...
function chooseResources(manifest, request, inspect = null) {
  ...
  const builderTask = /custom\s+(?:bricks\s+)?element|builder[-\s]?editable|editable\s+in\s+builder|configurable\s+in\s+builder|builder\s+controls?|set_controls|repeater|media\s+(?:control|library)|link\s+(?:control|picker)|shortcode\s+element|replace[^\n]{0,80}shortcode|migrate[^\n]{0,80}shortcode|select[^\n]{0,50}(?:products?|taxonomy|terms?)[^\n]{0,50}(?:builder|tabs?)|choose[^\n]{0,50}(?:products?|taxonomy|terms?)[^\n]{0,50}(?:builder|tabs?)|chọn\s+(?:thủ\s+công\s+)?sản\s*phẩm|chọn\s+(?:nhóm|loại|taxonomy|term)[^\n]{0,60}sản\s*phẩm|thay\s+đổi[^\n]{0,80}(?:tab\s*title|tab\s+title|content|nội\s+dung|hình\s+ảnh|image|link)|thêm\/xóa\/kéo|thêm[^\n]{0,40}xóa[^\n]{0,40}(?:tab|item)/.test(text);
