...
function streamWindow(job, key, requestedOffset) {
  ...
  return { text:job[key].slice(effective - base), offset:total, base, missed:requested < base };
}
...
function createTerminalRuntime(store, projects, { onChanged } = {}) {
  ...
  function defaultBroadcast(value) {
    try {
      for (const win of electron?.BrowserWindow?.getAllWindows?.() || []) {
        if (!win.isDestroyed?.()) win.webContents?.send?.('terminal:changed', value);
      }
    } catch {}
  }

  function emit(job) {
    const value = publicJob(job, { includeOutput:false });
    try { (typeof onChanged === 'function' ? onChanged : defaultBroadcast)(value); } catch {}
  }
  ...
  function prune() {
    ...
    const removable = [...jobs.values()].filter(job => !['running','stopping'].includes(job.status)).sort((a,b) => Date.parse(a.completedAt || a.createdAt) - Date.parse(b.completedAt || b.createdAt));
  ...
  function publicJob(job, { stdoutOffset = 0, stderrOffset = 0, includeOutput = true } = {}) {
    const out = streamWindow(job, 'stdout', stdoutOffset), err = streamWindow(job, 'stderr', stderrOffset);
    const active = ['running','stopping'].includes(job.status), successful = job.status === 'completed' || job.status === 'stopped';
    return {
  ...
  function armTimeout(job) {
    ...
    job.timeout = setTimeout(async () => {
      ...
      job.status = 'timeout'; job.stopReason = 'timeout';
  ...
  function list(projectRef = '') {
    ...
    return [...jobs.values()].filter(job => !projectId || job.projectId === projectId).sort((a,b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)).slice(0, 80).map(job => publicJob(job, { includeOutput:false }));
  }
  ...
  async function stop(jobId) {
    ...
    job.status = 'stopping'; job.stopReason = 'user'; emit(job);
    const stopped = await stopTree(job, true);
    if (!stopped && job.status === 'stopping') {
      job.status = 'failed'; job.completedAt = new Date().toISOString(); appendStream(job, 'stderr', '\nChatCode: không thể dừng process tree.'); emit(job);
    }
  ...
  electron?.app?.once?.('before-quit', () => { shutdown().catch(() => {}); });
...
function installTerminalRuntimePatches() {
  ...
  safetyModule.createSafeToolApi = function terminalAwareSafeToolApi(projects, store, approvals, backups, options) {
    ...
    const electron = electronApi();
    if (electron?.ipcMain?.handle && !electron.ipcMain.__chatcodeTerminalHandlers) {
      electron.ipcMain.__chatcodeTerminalHandlers = true;
      electron.ipcMain.handle('terminal:exec', (_, ref, command, opts) => api.exec(ref, command, opts || {}));
      electron.ipcMain.handle('terminal:status', (_, jobId, opts) => api.jobStatus(jobId, opts || {}));
      electron.ipcMain.handle('terminal:list', (_, ref) => api.listTerminalJobs(ref || ''));
      electron.ipcMain.handle('terminal:stop', (_, jobId) => api.jobStop(jobId));
    }
    return api;
  };
}

module.exports = { createTerminalRuntime, installTerminalRuntimePatches, commandGuard, isTrusted };
