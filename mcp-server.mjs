...
function buildMcpServer(api) {
  ...
  server.registerTool('prepare_task',{ title:'Prepare coding task', description:'Fast Agent Path call 1/2. Opens a Work Session and returns ranked source/Brain context, a compact Git baseline, saved project rules, verification hints and any mandatory built-in skill contract. For WordPress + Bricks projects this is the preferred entry and the returned wordpress-bricks instructions/resources are mandatory.', inputSchema:z.object({ project:z.string(), request:z.string().min(1).max(2000), limit:z.number().int().min(4).max(12).optional() }), annotations:LOCAL_SESSION_READ },wrap(api,'prepare_task',({project,request,limit})=>api.prepareTask(project,request,limit)));
  server.registerTool('complete_task',{ title:'Complete coding task', description:'Fast Agent Path call 2/2. Apply a transactional unified diff, run up to six verification commands, refresh Brain/Git and finalize the task. When commands are omitted, changed PHP and JavaScript files receive syntax checks automatically. Verification failure returns needs_fix while keeping the same task active. remember_project_rules is only for durable conventions or corrections explicitly confirmed by the user.', inputSchema:z.object({ task_id:z.string().min(1), patch:z.string().min(1).max(2097152), verify_commands:z.array(z.string().min(1).max(16000)).max(6).default([]), remember_project_rules:z.array(z.object({ key:z.string().min(1).max(64), value:z.string().min(1).max(600) })).max(12).default([]), finalize:z.boolean().default(true), rollback_on_failure:z.boolean().default(false) }), annotations:LOCAL_WRITE_OPEN_WORLD },wrap(api,'complete_task',({task_id,patch,verify_commands,remember_project_rules,finalize,rollback_on_failure})=>api.completeTask(task_id,patch,verify_commands,{ finalize, rollbackOnFailure:rollback_on_failure, rememberProjectRules:remember_project_rules })));

  server.registerTool('inspect_project',{ title:'Inspect project for a coding task', description:'Legacy/diagnostic context call. Returns framework/WordPress summary, relevant source, symbols, relations, Git state and mandatory skill policy when WordPress + Bricks is detected.', inputSchema:z.object({ project:z.string(), query:z.string().min(1), limit:z.number().int().min(4).max(16).optional() }), annotations:LOCAL_READ },wrap(api,'inspect_project',({project,query,limit})=>api.inspectProject(project,query,limit)));
  server.registerTool('apply_and_verify',{ title:'Apply changes and verify', description:'Legacy fast mutation path with recovery, Brain refresh, verification and Git diff. WordPress + Bricks skill policy must already be satisfied.', inputSchema:z.object({ project:z.string(), changes:z.array(changeSchema).max(24).default([]), tasks:z.array(z.string()).max(6).default([]) }), annotations:LOCAL_WRITE_OPEN_WORLD },wrap(api,'apply_and_verify',({project,changes,tasks})=>api.applyAndVerify(project,changes,tasks)));
  ...
  server.registerTool('apply_patch',{ title:'Apply unified diff', description:'Apply a standard multi-file unified diff transactionally. Preflights hunks, uses safety/recovery rules, refreshes Brain and returns Git state. WordPress + Bricks skill policy must already be satisfied.', inputSchema:z.object({ project:z.string(), patch:z.string().min(1).max(2097152), work_session_id:z.string().optional() }), annotations:LOCAL_WRITE },wrap(api,'apply_patch',({project,patch,work_session_id})=>api.applyPatch(project,patch,work_session_id||'')));
  server.registerTool('work_status',{ title:'Read work session', description:'Return session operations, changed files, commands, recovery points and current Git status/diff.', inputSchema:z.object({ work_session_id:z.string().min(1) }), annotations:LOCAL_READ },wrap(api,'work_status',({work_session_id})=>api.workStatus(work_session_id)));
  server.registerTool('finish_work',{ title:'Finish and verify work session', description:'Optionally run up to six verification commands, refresh Brain and return final Git diff/status.', inputSchema:z.object({ work_session_id:z.string().min(1), verify_commands:z.array(z.string().min(1)).max(6).default([]) }), annotations:LOCAL_WRITE_OPEN_WORLD },wrap(api,'finish_work',({work_session_id,verify_commands})=>api.finishWork(work_session_id,verify_commands)));
  server.registerTool('rollback_work',{ title:'Rollback work session', description:'Restore all file states changed by the work session in reverse order, including files created during the session, then refresh Brain and return Git state.', inputSchema:z.object({ work_session_id:z.string().min(1) }), annotations:LOCAL_WRITE },wrap(api,'rollback_work',({work_session_id})=>api.rollbackWork(work_session_id)));
  ...
  server.registerTool('git_status',{ title:'Git status', description:'Read Git branch/status.', inputSchema:z.object({ project:z.string() }), annotations:LOCAL_READ },wrap(api,'git_status',({project})=>api.gitStatus(project)));
  server.registerTool('git_diff',{ title:'Git diff', description:'Read unstaged or staged Git diff.', inputSchema:z.object({ project:z.string(), staged:z.boolean().optional() }), annotations:LOCAL_READ },wrap(api,'git_diff',({project,staged})=>api.gitDiff(project,!!staged)));
