...
function buildMcpServer(api) {
  const server = new McpServer({ name:'personal-chatcode', version:'0.9.0' }, { instructions:'ChatCode gives controlled access to local projects. Prefer inspect_project(query) as the first call for normal coding tasks: it returns framework summary, WordPress-aware context, relevant file contents, dependency graph and Git state. Prefer apply_and_verify(changes,tasks) for edits; it creates recovery snapshots, refreshes Project Brain, verifies mutations and returns git diff. If apply_and_verify returns status=pending, DO NOT retry it. Wait for the user to approve in ChatCode and then call operation_status(job_id). For Trusted Workspace, exec can run a real shell command inside a project cwd, including chaining/pipes and background jobs; use job_status for incremental output and job_stop to terminate a process tree. exec is not an OS filesystem sandbox, and Git push/reset --hard remain blocked. Safe Workspace keeps run_task allow-list behavior. Sensitive paths and file-tool project escapes remain blocked. Never push Git.' });
  ...
  server.registerTool('read_file',{ title:'Read file', description:'Read one UTF-8 text/code file. Unknown extensions are content-sniffed; sensitive paths are blocked unless explicitly enabled in Trusted Workspace.', inputSchema:z.object({ project:z.string(), path:z.string().min(1) }), annotations:{ readOnlyHint:true } },wrap(api,'read_file',({project,path})=>api.readFile(project,path)));
  ...
  server.registerTool('run_task',{ title:'Run development task', description:'Safe-mode compatible task runner: one allow-listed command, no shell chaining. Trusted Workspace may prefer exec for real terminal workflows.', inputSchema:z.object({ project:z.string(), command:z.string().min(1) }), annotations:{ readOnlyHint:false, destructiveHint:true } },wrap(api,'run_task',({project,command})=>api.runTask(project,command)));

  server.registerTool('exec',{ title:'Run Trusted terminal command', description:'Trusted Workspace only. Run a real shell command with cwd inside the project. Supports chaining/pipes and foreground or background jobs. This is not an OS filesystem sandbox. Git push/reset --hard and OS-destructive commands remain blocked.', inputSchema:z.object({ project:z.string(), command:z.string().min(1).max(16000), cwd:z.string().optional(), background:z.boolean().optional(), timeout_ms:z.number().int().min(1000).max(43200000).optional() }), annotations:{ readOnlyHint:false, destructiveHint:true } },wrap(api,'exec',({project,command,cwd,background,timeout_ms})=>api.exec(project,command,{ cwd,background,timeout_ms })));
  server.registerTool('job_status',{ title:'Read terminal job output', description:'Read current state and incremental stdout/stderr for a terminal job. Pass returned offsets on later calls to receive only new output.', inputSchema:z.object({ job_id:z.string().min(1), stdout_offset:z.number().int().min(0).optional(), stderr_offset:z.number().int().min(0).optional() }), annotations:{ readOnlyHint:true } },wrap(api,'job_status',({job_id,stdout_offset,stderr_offset})=>api.jobStatus(job_id,{ stdout_offset,stderr_offset })));
