...
function buildMcpServer(api){
  const server=new McpServer({name:'personal-chatcode',version:'0.8.0'},{instructions:'This server gives ChatGPT controlled access to folders explicitly added by the user. Start with list_projects. For unfamiliar or medium/large codebases, call project_brain or project_context before broad file reads; use find_symbols/find_references/related_files to narrow the working set. Use read/search tools before editing. Sensitive files and paths escaping the project through symlinks or junctions are blocked. Respect each project permission and Safety Rule. Some write/manage/task/git actions may pause while the user approves them in ChatCode Safety Center; do not retry the same tool call while approval is pending. After changes, inspect git diff and run relevant tests only when task permission is enabled.'});
  ...
  server.registerTool('project_brain',{title:'Project Brain overview',description:'Return cached code-intelligence summary for a project: detected frameworks/languages, entrypoints, symbol/dependency statistics, hotspots, and important symbols. Rebuilds automatically when the file index changed.',inputSchema:z.object({project:z.string()}),annotations:{readOnlyHint:true}},wrap(api,'project_brain',({project})=>api.projectBrain(project)));
  server.registerTool('find_symbols',{title:'Find code symbols',description:'Find functions, classes, interfaces, types, methods and other indexed symbols by name/path. Use this before reading many files.',inputSchema:z.object({project:z.string(),query:z.string().default(''),kind:z.string().optional(),limit:z.number().int().min(1).max(100).optional()}),annotations:{readOnlyHint:true}},wrap(api,'find_symbols',({project,query,kind,limit})=>api.findSymbols(project,query,kind||'',limit)));
  server.registerTool('find_references',{title:'Find symbol references',description:'Find indexed definitions and reference lines for a symbol across the project.',inputSchema:z.object({project:z.string(),symbol:z.string().min(1),limit:z.number().int().min(1).max(160).optional()}),annotations:{readOnlyHint:true}},wrap(api,'find_references',({project,symbol,limit})=>api.findReferences(project,symbol,limit)));
  server.registerTool('related_files',{title:'Find related files',description:'Rank files related to a target file using local import/dependency edges, reverse imports and module proximity.',inputSchema:z.object({project:z.string(),path:z.string().min(1),limit:z.number().int().min(1).max(50).optional()}),annotations:{readOnlyHint:true}},wrap(api,'related_files',({project,path,limit})=>api.relatedFiles(project,path,limit)));
  server.registerTool('project_context',{title:'Get task-focused project context',description:'Return a compact, ranked set of files/symbols/imports relevant to a task or question. Prefer this before broad read_files calls on unfamiliar projects.',inputSchema:z.object({project:z.string(),query:z.string().min(1),limit:z.number().int().min(3).max(24).optional()}),annotations:{readOnlyHint:true}},wrap(api,'project_context',({project,query,limit})=>api.projectContext(project,query,limit)));
