...
function buildMcpServer(api){
  const server=new McpServer({name:'personal-chatcode',version:'0.6.0'},{instructions:'This server gives ChatGPT controlled access to folders explicitly added by the user. Start with list_projects. Use read/search tools before editing. Sensitive files and paths escaping the project through symlinks or junctions are blocked. Respect each project permission. After changes, inspect git diff and run relevant tests only when task permission is enabled.'});
  ...
  server.registerTool('write_file',{title:'Create or replace file',description:'Create or replace a UTF-8 text file. Requires Write permission.',inputSchema:z.object({project:z.string(),path:z.string().min(1),content:z.string()}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'write_file',({project,path,content})=>api.writeFile(project,path,content)));
  server.registerTool('delete_file',{title:'Delete file',description:'Delete one file. Requires Manage Files permission.',inputSchema:z.object({project:z.string(),path:z.string().min(1)}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'delete_file',({project,path})=>api.deleteFile(project,path)));
  server.registerTool('rename_file',{title:'Rename file',description:'Rename or move one file within the same project. Requires Manage Files permission.',inputSchema:z.object({project:z.string(),from:z.string().min(1),to:z.string().min(1)}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'rename_file',({project,from,to})=>api.renameFile(project,from,to)));
  server.registerTool('run_task',{title:'Run development task',description:'Run an allow-listed development command such as npm test, npm run build, pytest, cargo test, go test, or dotnet test. Requires Task permission.',inputSchema:z.object({project:z.string(),command:z.string().min(1)}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'run_task',({project,command})=>api.runTask(project,command)));
  ...
  server.registerTool('git_stage',{title:'Stage git files',description:'Stage explicit non-sensitive file paths. Requires Git write permission.',inputSchema:z.object({project:z.string(),paths:z.array(z.string().min(1)).min(1).max(100)}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'git_stage',({project,paths})=>api.gitStage(project,paths)));
  server.registerTool('git_commit',{title:'Create local git commit',description:'Commit already staged changes locally. Requires Git write permission. Never pushes.',inputSchema:z.object({project:z.string(),message:z.string().min(1).max(300)}),annotations:{readOnlyHint:false,destructiveHint:true}},wrap(api,'git_commit',({project,message})=>api.gitCommit(project,message)));
  return server;
}
